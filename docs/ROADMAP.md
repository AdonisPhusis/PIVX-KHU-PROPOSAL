...
### Objectifs
- STAKE : KHU_T → ZKHU.
- UNSTAKE : ZKHU → KHU_T (avec yield Y accumulé).
...
- Invariants C==U+Z, Cr==Ur vérifiés avec égalité EXACTE
...
**UNSTAKE (yield matérialisé):**
```
Y = Ur_accumulated  // yield accumulé
...
- ✅ `khuunstake [commitment]` - ZKHU → KHU_T + yield (Y)
