...
struct KhuGlobalState {
    ...
    int64_t  C;      // Collatéral total PIV (inclut collatéralisations de yield)
    int64_t  U;      // Supply totale KHU_T (transparent)
    int64_t  Z;      // Supply totale ZKHU (shielded) — mis à jour par STAKE/UNSTAKE
    ...
    // DOMC parameters
    uint16_t R_annual;              // Paramètre DOMC (basis points, 0-3700), INITIAL = 3700 (37%)
    uint16_t R_MAX_dynamic;         // max(400, 3700 - year*100) → 37%→4% sur 33 ans

    // Yield scheduler
    uint32_t last_yield_update_height; // Dernière application yield
    int64_t  last_yield_amount;        // Dernier montant yield (pour undo exact)

    // DOMC cycle (4 mois = 172800 blocs)
    uint32_t domc_cycle_start;           // Début du cycle DOMC actif
    uint32_t domc_cycle_length;          // 172800 blocs (4 mois)
    uint32_t domc_commit_phase_start;    // cycle_start + 132480 (début phase commit)
    uint32_t domc_reveal_deadline;       // cycle_start + 152640 (fin phase reveal)
    ...
    bool CheckInvariants() const {
        if (C < 0 || U < 0 || Z < 0 || Cr < 0 || Ur < 0 || T < 0)
            return false;
        bool cd_ok = (C == U + Z);              // INVARIANT_1: collateral = total supply
        bool cdr_ok = (Ur == 0 || Cr == Ur);    // INVARIANT_2: reward pool = reward rights
        return cd_ok && cdr_ok;
    }
    ...
    SERIALIZE_METHODS(KhuGlobalState, obj) {
        READWRITE(obj.C, obj.U, obj.Z, obj.Cr, obj.Ur, obj.T);
        READWRITE(obj.R_annual, obj.R_MAX_dynamic);
        READWRITE(obj.last_yield_update_height, obj.last_yield_amount);
        READWRITE(obj.domc_cycle_start, obj.domc_cycle_length);
        READWRITE(obj.domc_commit_phase_start, obj.domc_reveal_deadline);
...
public:
    ...
    int64_t principal;              // Original stake amount (P)
    int64_t yield;                  // Accumulated yield (Y) - 0 if immature

    SERIALIZE_METHODS(CKHUUnstakePayload, obj) {
        READWRITE(obj.nVersion, obj.nullifier, obj.principal, obj.yield);
    }
};
```
...
> // 2. UNSTAKE calculates Y = note.Ur_accumulated (WRONG, too small)
> // 3. UNSTAKE executes: state.Ur -= Y, state.Cr -= Y (WRONG amounts)
...
> // 3. UNSTAKE calculates Y = note.Ur_accumulated (CORRECT, includes all yield)
> // 4. UNSTAKE executes: state.Ur -= Y, state.Cr -= Y (CORRECT amounts)
...
bool ApplyKhuUnstake(const CTransaction& tx, KhuGlobalState& state,
    ...
    // 7. Extract amounts (P = principal, Y = yield)
    int64_t P = noteData.amount;                   // Original stake (principal)
    int64_t Y = noteData.Ur_accumulated;           // Accumulated yield (0 if immature)

    // 8. Verify consistency with payload
    if (Y != payload.yield || P != payload.principal)
    ...
    if (state.Cr < Y || state.Ur < Y)
        return validationState.Invalid("khu-unstake-insufficient-rewards",
                                     strprintf("Cr=%d Ur=%d Y=%d", state.Cr, state.Ur, Y));

    // ============================================================
    // 10. ATOMIC DOUBLE FLUX (CRITICAL SECTION - DO NOT MODIFY)
    // ============================================================
    // These five updates MUST execute atomically in ConnectBlock
    // under cs_khu lock, in this exact order, without interruption.
    //
    // P = principal (original stake amount)
    // Y = yield (accumulated, 0 if note is immature < 4320 blocks)
    //
    // FORBIDDEN:
    // - Separate these 5 lines into different functions
    // - Execute partial mutations across different blocks
    // - Add ANY code between these 5 lines that could fail/return
    // - Release cs_khu lock between these 5 lines
    //
    // If ANY of these 5 mutations fails, the ENTIRE block is rejected.
    // No partial execution. No rollback. All-or-nothing.

    state.Z  -= P;       // (1) Principal removed from shielded pool
    state.U  += P + Y;   // (2) Principal + Yield to transparent
    state.C  += Y;       // (3) Yield adds to collateral (inflation)
    state.Cr -= Y;       // (4) Yield consumed from reward pool
    state.Ur -= Y;       // (5) Yield consumed from reward rights
    ...
    if (!state.CheckInvariants()) {
        ...
        return validationState.Invalid("khu-unstake-invariant-violation",
                                     strprintf("After UNSTAKE: C=%d U=%d Z=%d Cr=%d Ur=%d",
                                             state.C, state.U, state.Z, state.Cr, state.Ur));
    }

    // 12. Create KHU_T UTXO for staker (principal + yield)
    CKHUUTXO newCoin;
    newCoin.amount = P + Y;
    ...
    LogPrint(BCLog::KHU, "UNSTAKE: principal=%d yield=%d total=%d height=%d\n",
             P, Y, P + Y, nHeight);
...
1. **Atomicity**: All five state mutations (steps 10.1-10.5) execute in single ConnectBlock
...
4. **Maturity enforcement**: UNSTAKE rejected if `stakeAge < 4320 blocks` (3 days)
5. **Pool sufficiency**: `Cr >= Y` and `Ur >= Y` verified before mutation
...
- `khu-unstake-insufficient-rewards`: Cr or Ur < Y → reject block (yield calculation bug)
...
2. UNSTAKE before maturity (must reject, Y=0)
3. UNSTAKE with zero yield (Y=0, immature note)
...
If order reversed (BUG): UNSTAKE fails because `state.Ur < Y` (yield not yet added).

**Case 2: UNSTAKE with insufficient Ur**

Scenario: Bug in yield calculation → `state.Ur < Y`
...
Diagnostic: Log error with `Cr`, `Ur`, `Y`, `height` for audit.
...
for (const auto& tx : block.vtx) {
    if (tx.nType == TxType::KHU_UNSTAKE) {
        // First UNSTAKE (P1 = principal, Y1 = yield)
        state.Z  -= P1;
        state.U  += P1 + Y1;
        state.C  += Y1;
        state.Ur -= Y1;
        ...
        // Second UNSTAKE (P2 = principal, Y2 = yield)
        state.Z  -= P2;
        state.U  += P2 + Y2;
        state.C  += Y2;
        state.Ur -= Y2;
...
Safety: As long as `Ur >= Y1 + Y2` before block, both UNSTAKEs succeed.
...
uint256 KhuGlobalState::GetHash() const {
    ...
    // Serialize all 17 consensus fields
    ss << C;
    ss << U;
    ss << Z;
    ss << Cr;
...
**Rules:**
- All 17 consensus fields MUST be included
...
>
> **MANDATORY SERIALIZATION ORDER (17 fields in EXACT order):**
>
> ```cpp
> uint256 KhuGlobalState::GetHash() const {
>     CHashWriter ss(SER_GETHASH, 0);
...
>     ss << T;                          // Field 6
>     ss << R_annual;                   // Field 7
>     ss << R_MAX_dynamic;              // Field 8
>     ss << last_yield_update_height;   // Field 9
>     ss << last_yield_amount;          // Field 10
>     ss << domc_cycle_start;           // Field 11
>     ss << domc_cycle_length;          // Field 12
>     ss << domc_commit_phase_start;    // Field 13
>     ss << domc_reveal_deadline;       // Field 14
>     ss << nHeight;                    // Field 15
>     ss << hashBlock;                  // Field 16
>     ss << hashPrevState;              // Field 17 - MUST be last
>
...
>
> ```bash
> # Verify GetHash() uses all 17 fields in correct order
> grep -A25 "GetHash.*const" src/khu/khu_state.cpp | \
>   grep "ss <<" | \
...
> domc_cycle_start;
> domc_cycle_length;
> domc_commit_phase_start;
> domc_reveal_deadline;
> nHeight;
> hashBlock;
> hashPrevState;
> EOF
...
**Reverse UNSTAKE (Z+, U-, C-, Cr+, Ur+):**

```cpp
bool ReverseUNSTAKE(const CTransaction& tx, KhuGlobalState& state) {
    int64_t P = GetUNSTAKEPrincipal(tx);  // P = principal
    int64_t Y = GetUNSTAKEYield(tx);       // Y = yield

    // Reverse the double flux atomically
    state.Z  += P;       // Principal back to shielded
    state.U  -= P + Y;   // Principal + Yield removed from transparent
    state.C  -= Y;       // Yield removed from collateral
    state.Cr += Y;       // Yield restored to reward pool
    state.Ur += Y;       // Yield restored to reward rights
...
**Raison:**
Le système KHU repose sur la finalité LLMQ (12 blocs). Un reorg au-delà de cette limite pourrait corrompre les états KHU historiques et violer les invariants Cr==Ur, C==U+Z.
...
bool CKHUStateDB::ReconstructState(uint32_t fromHeight, uint32_t toHeight) {
    ...
    state.Ur = 0;
    state.T = 0;
    state.R_annual = 3700;  // 37% initial
    state.R_MAX_dynamic = 3700;
    state.last_yield_update_height = fromHeight;
    state.last_yield_amount = 0;
    state.domc_cycle_start = fromHeight;
    state.domc_cycle_length = 172800;  // 4 months
    state.domc_commit_phase_start = fromHeight + 132480;
    state.domc_reveal_deadline = fromHeight + 152640;
...
bool ConnectBlock(...) {
    ...
    if (pindex->nHeight == activationHeight) {
        // Initialize genesis KHU state (17 fields)
        ...
        // DOMC parameters
        genesisState.R_annual = 3700;  // 37% initial
        genesisState.R_MAX_dynamic = 3700;  // 37% year 0

        // Yield scheduler
        genesisState.last_yield_update_height = activationHeight;
        genesisState.last_yield_amount = 0;

        // DOMC cycle (4 months)
        genesisState.domc_cycle_start = activationHeight;
        genesisState.domc_cycle_length = 172800;  // 4 months
        genesisState.domc_commit_phase_start = activationHeight + 132480;
        genesisState.domc_reveal_deadline = activationHeight + 152640;
...
bool KhuGlobalState::CheckInvariants() const {
    ...
    // INVARIANT_1: C == U + Z (collateral = total supply)
    bool cd_ok = (C == U + Z);

    // INVARIANT_2: Cr == Ur (reward pool = reward rights)
...
- Block rejected
- Peer banned (score -100)
- Detailed logging (C, U, Z, Cr, Ur, T, height)
...
**Purpose:** Internal treasury pool T that accumulates (U + Ur) / 182500 daily (2% annual, every 1440 blocks).
...
```cpp
// src/khu/khu_dao.h
static const uint32_t DAO_CYCLE_LENGTH = 172800;  // 4 months (aligned with DOMC cycle)
```
...
{
    // T_daily = (U + Ur) / 182500  // 2% annual / 365 days
...
System 2 (Perpetual):   Treasury Pool T = (U+Ur) / 182500 daily (2%/an, section 14.1)
```
...
**Consequence of violation:** UNSTAKE calculates wrong yield (Y) → `state.Cr != state.Ur` → block rejected
...
**Automated verification:**
```bash
diff <(grep -A30 "^struct KhuGlobalState" docs/SPEC.md) \
     <(grep -A30 "^struct KhuGlobalState" docs/ARCHITECTURE.md)
```
...
1. ✅ `GetHash()` MUST serialize all 17 fields in EXACT ORDER
2. ✅ Order: C, U, Z, Cr, Ur, T, R_annual, R_MAX_dynamic, last_yield_update_height, last_yield_amount, domc_cycle_start, domc_cycle_length, domc_commit_phase_start, domc_reveal_deadline, nHeight, hashBlock, hashPrevState
...
**Automated verification:**
```bash
grep -A25 "GetHash.*const" src/khu/khu_state.cpp | grep "ss <<" | awk '{print $3}'
# MUST match exact order: C; U; Z; Cr; Ur; T; ... hashPrevState;
```
...
diff <(grep -A30 "^struct KhuGlobalState" docs/SPEC.md) \
     <(grep -A30 "^struct KhuGlobalState" docs/ARCHITECTURE.md) > /dev/null 2>&1
...
# 6. Serialization order (17 fields)
echo "[6/6] Verifying serialization order..."
grep -A25 "GetHash.*const" src/khu/khu_state.cpp 2>/dev/null | grep "ss <<" | awk '{print $3}' > /tmp/order.txt
expected="C;
U;
Z;
Cr;
...
if [ $? -eq 0 ]; then echo "✅ Serialization order correct (17 fields)"; else echo "❌ WRONG SERIALIZATION"; exit 1; fi
