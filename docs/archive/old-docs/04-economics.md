...
**Z est stocké dans KhuGlobalState** et mis à jour atomiquement par STAKE (+) et UNSTAKE (-).
...
où:
  ...
  Z = ZKHU stakés (shielded, stocké dans state)
```
...
Pre-state:   (C, U, Z, Cr, Ur)  où Z est stocké dans state
Post-state:  (C', U', Z', Cr', Ur')

Equations:
C'  = C
U'  = U - amount      // KHU_T supply decreases
Z'  = Z + amount      // ZKHU supply increases (stored in state)
Cr' = Cr
Ur' = Ur
```
...
où Z = state.Z (stocké dans KhuGlobalState)
...
```
Theorem 1:  ∀h ≥ h₀,  C(h) = U(h) + Z(h)    où Z(h) = state.Z
