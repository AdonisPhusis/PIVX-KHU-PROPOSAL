...
bool ConnectBlock(...) {
    ...
    // 5. CheckInvariants() — C==U+Z and Cr==Ur
...
**Problème:**
Doc 02 section 2.3 disait "C == U (strict equality)" mais a été corrigé à `C == U + Z` (collateral = transparent + shielded supply).
...
```
INVARIANT_1: C == U + Z  (collateral = transparent + shielded supply)
INVARIANT_2: Cr == Ur    (reward pool = reward rights)
```

**Note ajoutée:**
"Au genesis, C=U=Z=0 est autorisé. L'invariant C == U + Z doit tenir à tout moment. STAKE déplace de U vers Z, UNSTAKE déplace de Z vers U (avec bonus)."
