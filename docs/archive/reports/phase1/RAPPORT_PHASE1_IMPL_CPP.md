...
- ✅ **Invariants**: C==U+Z et Cr==Ur implémentés et vérifiés
...
bool CheckInvariants() const
{
    ...
    if (C < 0 || U < 0 || Z < 0 || Cr < 0 || Ur < 0 || T < 0) return false;

    // C/U/Z invariant: C == U + Z (collateral = transparent + shielded supply)
    bool cuz_ok = (C == U + Z);

    // Cr/Ur invariant: Cr == Ur (reward pool = reward rights)
    bool crur_ok = (Cr == Ur);
...
| 2 | `test_invariants_cuz` | C==U+Z invariant validation |
...
```cpp
BOOST_AUTO_TEST_CASE(test_invariants_cuz)
{
    KhuGlobalState state;
    state.SetNull();

    // Valid: C == U + Z == 0 (genesis)
    BOOST_CHECK(state.CheckInvariants());

    // Valid: C == U + Z == 1000 (no staking)
    state.C = 1000 * COIN;
    state.U = 1000 * COIN;
    state.Z = 0;
    BOOST_CHECK(state.CheckInvariants());

    // Valid: C == U + Z (with staking)
    state.C = 1000 * COIN;
    state.U = 700 * COIN;
    state.Z = 300 * COIN;
    BOOST_CHECK(state.CheckInvariants());
...
✅ **Protection**: Tout bloc violant C==U+Z ou Cr==Ur est **REJETÉ** par consensus.
