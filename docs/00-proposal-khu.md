...
| **Z** | Supply Shielded | ZKHU staké = Σ(notes actives) | int64_t (calculé) |
...
**Note:** Z n'est pas stocké dans KhuGlobalState — il est **calculé dynamiquement** via `GetTotalStakedZKHU()` en itérant les notes ZKHU actives.
...
struct KhuGlobalState {
    ...
    // NOTE: Z = Σ(ZKHU notes actives) — calculé via GetTotalStakedZKHU()
    ...
    bool CheckInvariants() const {
        int64_t Z = GetTotalStakedZKHU();  // Calculé dynamiquement
        ...
        // INVARIANT 3: Treasury non-négatif
        bool treasury_ok = (T >= 0);  // Déjà vérifié ci-dessus

        return principal_ok && reward_ok && treasury_ok;
    }
};
```
...
// Z += amount  // ZKHU note créée (shielded supply ↑) — implicite via DB
...
// Z -= P    // Note ZKHU détruite (implicite via nullifier)
