...
bool GetMintKHUPayload(const CTransaction& tx, CMintKHUPayload& payload)
{
    ...
    try {
        ...
        LogPrint(BCLog::KHU, "ERROR: GetMintKHUPayload: %s\n", e.what());
        return false;
    }
}
...
bool ApplyKHUMint(const CTransaction& tx, KhuGlobalState& state, CCoinsViewCache& view, uint32_t nHeight)
{
    ...
    CMintKHUPayload payload;
    if (!GetMintKHUPayload(tx, payload)) {
        return error("ApplyKHUMint: Failed to extract payload");
    }
    ...
    if (!state.CheckInvariants()) {
        return error("ApplyKHUMint: Pre-invariant violation (C=%d U=%d Cr=%d Ur=%d)",
    ...
    if (!state.CheckInvariants()) {
        return error("ApplyKHUMint: Post-invariant violation (C=%d U=%d Cr=%d Ur=%d)",
                     state.C, state.U, state.Cr, state.Ur);
    }
    ...
    LogPrint(BCLog::KHU, "%s: added KHU coin %s:%d value=%s (C=%s, U=%s)\n",
...
bool UndoKHUMint(const CTransaction& tx, KhuGlobalState& state, CCoinsViewCache& view)
{
    // ⚠️ CRITICAL: cs_khu MUST be held
    AssertLockHeld(cs_khu);
