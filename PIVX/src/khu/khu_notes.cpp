...
ZKHUMemo ZKHUMemo::Encode(const ZKHUNoteData& noteData)
{
    ...
    uint32_t height_le = htole32(noteData.nStakeStartHeight);
...
Optional<ZKHUNoteData> ZKHUMemo::Decode(const std::array<unsigned char, MEMO_SIZE>& memo, const uint256& nullifier, const uint256& cm)
{
    ...
    uint32_t nStakeStartHeight = le32toh(height_le);
    offset += sizeof(uint32_t);
    ...
    ZKHUNoteData noteData(amount, nStakeStartHeight, Ur_accumulated, nullifier, cm);
    return noteData;
}
